#!/bin/bash
# BackupBear Master Packager 13.8.1: The Cron Fix
...
echo "🐻 BackupBear 13.8.1 Compiler & Packager"
echo "========================================"
...
if [ -f "logo.png" ]; then
    if command -v magick &> /dev/null; then 
        magick "logo.png" -resize 512x512 "build/icons/512x512.png"
    elif command -v convert &> /dev/null; then 
        convert "logo.png" -resize 512x512 "build/icons/512x512.png"
    else
...
ipcMain.handle('check-deps', async () => {
    ...
    let installCmd = 'sudo apt install rclone rsync cron';
    if (checkCmd('pacman')) installCmd = 'sudo pacman -S rclone rsync cronie && sudo systemctl enable --now cronie.service';
    else if (checkCmd('dnf')) installCmd = 'sudo dnf install rclone rsync cronie && sudo systemctl enable --now crond.service';
    
    return { ok: missing.length === 0, missing, installCmd };
});

// SCHEDULE HANDLERS (CRON + HEADLESS BASH GENERATOR)
...
ipcMain.handle('save-schedule', (e, payload) => {
    try {
        const { sched, sources, destinations, encrypt, key, mode, excludes } = payload;
        ...
        // 1. Safely extract existing Crontab (excluding the "no crontab" warning)
        let currentCron = "";
        ...
        // 2. Generate the Headless Bash Runner Script
        let script = `#!/bin/bash\n`;
        script += `export DISPLAY=:0\nexport DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/$(id -u)/bus\n\n`;
        script += `LOG="${sched.logEnabled ? sched.logPath : '/dev/null'}"\n\n`;
        script += `echo "=====================================" >> "$LOG"\n`;
        script += `echo "🐻 BackupBear Scheduled Run: $(date)" >> "$LOG"\n`;
        script += `notify-send "BackupBear" "Scheduled backup started..." -u normal || true\n\n`;
        ...
        script += `echo "Finished: $(date)" >> "$LOG"\n`;

        fs.writeFileSync(runnerScript, script, { mode: 0o755 });

        // 3. Inject into crontab
...
<body>
    <div id="dep-overlay">
        ...
        <p class="text-secondary fs-6 mb-3">BackupBear requires <b class="text-white">rclone</b>, <b class="text-white">rsync</b>, and <b class="text-white">cron</b> to function.</p>
    ...
    <div class="content">
        ...
        <div id="view-schedule" class="view-section">
            ...
            <div class="card p-3 border-secondary mb-3">
                <p class="small text-warning fw-bold mb-3"><i class="bi bi-info-circle"></i> To automate backups, set up your Sources, Destinations, and Excludes on the other tabs first. This tool takes a "snapshot" of your current configuration and builds a script for Linux Crontab to run in the background.</p>
        ...
        <div id="view-about" class="view-section text-center">
            <div style="margin-top: 20px;">
                ...
                <div class="card d-inline-block p-3 text-start border-secondary mb-4" style="min-width: 250px;">
                    <div class="mb-2"><span class="text-secondary fw-bold small">VERSION</span><br><span class="fw-bold text-white fs-6">13.8.1</span></div>
                    ...
                    <div><span class="text-secondary fw-bold small">ENGINE</span><br><span class="font-monospace text-white fs-6">Electron + Rclone + Rsync + Cron</span></div>
                </div>
                <div>
...
document.getElementById('btn-save-cron').addEventListener('click', async () => {
    ...
    const payload = {
        ...
        encrypt, key, mode, excludes: activeExcludes
    };
...
document.getElementById('btn-run-backup').addEventListener('click', () => {
    ...
    const encrypt = document.getElementById('bak-enc').checked; const key = document.getElementById('bak-key').value; const mode = document.getElementById('mode-full').checked ? 'full' : 'inc';
    if (encrypt) { if(!key) return alert("Enter a passphrase"); if(key !== document.getElementById('bak-key-confirm').value) return alert("Passphrases do not match!"); }
    executeBackup(sources, destinations, encrypt, key, mode);
});

document.getElementById('btn-force-backup').addEventListener('click', () => { bootstrap.Modal.getInstance(document.getElementById('weakPwModal')).hide(); const mode = document.getElementById('mode-full').checked ? 'full' : 'inc'; executeBackup(sources, destinations, true, document.getElementById('bak-key').value, mode); });
...
echo "📝 Updating package.json to v13.8.1..."
cat <<EOF > package.json
{
  "name": "backupbear",
  "productName": "BackupBear",
  "version": "13.8.1",
  "description": "Secure, reliable system backup utility.",
  "main": "main.js",
  "author": "Aureus The SpaceBearTaur",
  "license": "MIT",
  "homepage": "none",
  "scripts": {
  ...
  "build": {
    ...
    "linux": {
      ...
      "maintainer": "Aureus the SpaceBearTaur <aureus@localhost>",
      "synopsis": "Automated Backup Utility"
    },
    "files": [
      "**/*",
      "!dist/*",
...
echo "🔨 Compiling v13.8.1 Packages..."
